    ...
    for ($idx=$table_index; $idx<count($search_table); $idx++) {
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            if (strstr($row['wr_option'].$row2['wr_option'], 'secret'))
                $row['wr_content'] = '[this is a secret.]';
...
$group_select = '<label for="gr_id" class="sound_only">Board Group Selection</label><select name="gr_id" id="gr_id" class="select"><option value="">Full classification';
