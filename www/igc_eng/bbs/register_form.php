...
if ($w == "") {
    ...
    if (!isset($_POST['agree']) || !$_POST['agree']) {
        alert('You must agree to the terms and conditions of membership in order to register.', G5_BBS_URL.'/register.php');
    }

    if (!isset($_POST['agree2']) || !$_POST['agree2']) {
        alert('You must agree to the contents of the personal information processing policy to register as a member.', G5_BBS_URL.'/register.php');
    }
    ...
    if ($is_admin)
        alert('Please modify the administrator member information on the administrator screen.', G5_URL);

    if (!$is_member)
        alert('Please use after logging in.', G5_URL);

    if ($member['mb_id'] != $_POST['mb_id'])
        alert('The logged-in member and the transferred information are different.');
    ...
    */
    ...
    $g5['title'] = 'Member information modification';
    ...
    alert('w The price did not go over.');
}
