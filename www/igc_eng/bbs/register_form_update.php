...
if (!($w == '' || $w == 'u')) {
    alert('w The value did not pass correctly.');
}

if ($w == 'u' && $is_admin == 'super') {
    if (file_exists(G5_PATH.'/DEMO'))
        alert('This is something you cannot (see) on the demo screen.');
}

if (!chk_captcha()) {
    alert('The number of automatic registration prevention is incorrect.');
}
...
if(!$mb_id)
    alert('There is no member ID value. Please use it the right way.');
...
if ($w == '' || $w == 'u') {
    ...
    if ($w == '' && !$mb_password)
        alert('The password has not been passed.');
    ...
    if ($w=='') {
        ...
        if (get_session('ss_check_mb_id') != $mb_id || get_session('ss_check_mb_nick') != $mb_nick || get_session('ss_check_mb_email') != $mb_email) {
            ...
            alert('Please use it the right way.');
        }
...
if($config['cf_cert_use'] && $_SESSION['ss_cert_type'] && $_SESSION['ss_cert_dupinfo']) {
    ...
    $row = sql_fetch($sql);
    if ($row['mb_id']) {
        alert("There is a subscription history with the entered identification information..\\nMember ID : ".$row['mb_id']);
    }
}
...
if ($w == '') {
    ...
    // 회원가입 포인트 부여
    insert_point($mb_id, $config['cf_register_point'], 'Membership celebration', '@member', $mb_id, 'Sign Up');

    // 추천인에게 포인트 부여
    if ($config['cf_use_recommend'] && $mb_recommend)
        insert_point($mb_recommend, $config['cf_recommend_point'], $mb_id.'Recommender', '@member', $mb_recommend, $mb_id.' Recommendation');

    // 회원님께 메일 발송
    if ($config['cf_email_mb_member']) {
        $subject = '['.$config['cf_title'].'] Congratulations on registering as a member.';
    ...
    if ($config['cf_email_mb_super_admin']) {
        $subject = run_replace('register_form_update_mail_admin_subject', '['.$config['cf_title'].'] '.$mb_nick .' You have signed up as a member.', $mb_id, $mb_nick);
    ...
    if (trim($_POST['mb_id']) != $mb_id)
        alert("The information you are logged in and the information you want to edit are different and cannot be modified..\\nIf you use the wrong method, please stop immediately.");
...
if (isset($_FILES['mb_icon']) && is_uploaded_file($_FILES['mb_icon']['tmp_name'])) {
    if (preg_match($image_regex, $_FILES['mb_icon']['name'])) {
        ...
        if ($_FILES['mb_icon']['size'] <= $config['cf_member_icon_size']) {
            ...
            $msg .= '회원아이콘을 '.number_format($config['cf_member_icon_size']).'Please upload in bytes or less.';
        }
    ...
    if (isset($_FILES['mb_img']) && is_uploaded_file($_FILES['mb_img']['tmp_name'])) {
        ...
        if (preg_match($image_regex, $_FILES['mb_img']['name'])) {
            ...
            if ($_FILES['mb_img']['size'] <= $config['cf_member_img_size']) {
                ...
                $msg .= '회원이미지을 '.number_format($config['cf_member_img_size']).'Please upload in bytes or less.';
            }
...
if ($config['cf_use_email_certify'] && $old_email != $mb_email) {
    $subject = '['.$config['cf_title'].'] This is the verification email.';
...
if ($w == '') {
    ...
    if ($old_email != $mb_email && $config['cf_use_email_certify']) {
        ...
        alert('Member information has been modified.\n\nYour e-mail address has changed and you will need to re-authenticate.', G5_URL);
    } else {
        echo '
        <!doctype html>
        <html lang="ko">
