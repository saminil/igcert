<?php
if (!defined('_GNUBOARD_')) exit; // 개별 페이지 접근 불가

if (G5_IS_MOBILE) {
    include_once(G5_THEME_MOBILE_PATH.'/head.php');
    return;
}

include_once(G5_THEME_PATH.'/head.sub.php');
include_once(G5_LIB_PATH.'/latest.lib.php');
include_once(G5_LIB_PATH.'/outlogin.lib.php');
include_once(G5_LIB_PATH.'/poll.lib.php');
include_once(G5_LIB_PATH.'/visit.lib.php');
include_once(G5_LIB_PATH.'/connect.lib.php');
include_once(G5_LIB_PATH.'/popular.lib.php');
?>

<!----------- 헤더 시작 { ----------->
<header id="header">
    <div class="dimmed"></div>
    <div class="wrap">
		<?php
		if(defined('_INDEX_')) { // index에서만 실행
			include G5_BBS_PATH.'/newwin.inc.php'; // 팝업레이어
		}
		?>	
			
		<h1><a href="/igc_eng/" title="To Main page"><!------Company Logo-----></a></h1>
		
        <!--
		<?php if(defined('_INDEX_')) { ?>
		    <div class="pc_ci pc_only">
                <a href="/igc_eng/" title="To Main page"><img src="<?php echo G5_THEME_URL ?>/img/ci_img_log.png" alt="logo" class="logo" /></a>//서브 로고(상단 좌측, 하단 좌측:footer)
			    <div class=logo_link_t_einglish>
                    <h3 class="logo_link_tt_einglish_h3"> 
			            <span>Dream <br>with IGC</span>
                    </h3>
               
                    <hr class="hr01">
                
                    <h3 class="logo_link_t_einglish_h3"> 
			            <span><a href="/" style="padding:45px;top:40px;color:#333;">Korean</a></span>
                    </h3>
			    </div>
			    <a href="http://www.igcert.net" target="_blank"><img src="<?php echo G5_THEME_URL ?>/img/ci_inquiry.png" alt="See previous Website" /></a>
			    <a href="#certification_search" style="display:inline-block;margin-top:0px;"><img src="<?php echo G5_THEME_URL ?>/img/ci_location.png" alt="Certificate Search" /></a>
            </div>
		<? } ?>
        -->
    
       <!----------- PC버전 국문페이지링크 버튼 ----------->
        <button class="button_001" type="button" onclick="location.href='/'" title="To Korean/국문사이트"><span class="to_korean">KOR</span></button>	
        
        <!----------- PC버전 SNS 버튼 영역 ----------->
		<nav class="sns mobile_no">
			<ul>
				<li><a id="channel-chat-button" href="http://pf.kakao.com/_MdxlzK/chat" onclick="void chatChannel();" title="KAKAO Talk">KAKAO Talk</a></li>
                <!--
				<li><a href="javascript:">English</a></li>
				<li><a href="javascript:">유튜브</a></li> 
				-->
			</ul>
		</nav>
       
        <!----------- 회원메뉴 영역 ----------->
		<a href="javascript:" class="btn_user pc_only" title="Member Menu"><i class="fa fa-user-o" aria-hidden="true"></i></a>
		<a href="<?php echo G5_URL ?>/bbs/login.php" class="btn_user pc_no" title="Member Menu"><i class="fa fa-user-o" aria-hidden="true"></i></a>
		<nav class="user">
			<ul>
				<?php if ($is_member) {  ?>
				<li><a href="<?php echo G5_BBS_URL ?>/member_confirm.php?url=<?php echo G5_BBS_URL ?>/register_form.php">My page</a></li>
				<li><a href="<?php echo G5_BBS_URL ?>/logout.php">Log Out</a></li>				
				<?php } else {  ?>				
				<li><a href="<?php echo G5_BBS_URL ?>/login.php">Log In</a></li>
				<li><a href="<?php echo G5_BBS_URL ?>/register.php">Sign Up</a></li>
				<?php }  ?>
				<?php if ($is_admin) {  ?>
				<li class="admin"><a href="<?php echo correct_goto_url(G5_ADMIN_URL); ?>" target="_blank">Admin</a></li>
				<?php }  ?>	
			</ul>
		</nav>
		
		<!----------- 검색 버튼 ----------->
		<a href="javascript:" class="btn_search" title="Search"><i class="fa fa-search" aria-hidden="true"></i></a>
		
		<!----------- 태블릿/모바일버전 GNB 버튼 ----------->
		<a href="javascript:" class="btn_gnb mobile_only" title="Mobile Menu Open/Close"><i class="fa fa-bars" aria-hidden="true"></i></a>
		
		<!----------- 메인 메뉴 영역 ----------->
		<nav class="gnb">
			<ul>
			
				<?php
				$menu_datas = get_menu_db(0, true);
				$cur_url = $_SERVER['REQUEST_URI'];          
				$gnb_zindex = 999; // gnb_1dli z-index 값 설정용
                $i = 0; 		
				foreach( $menu_datas as $row ){
				?>
				
				<li class="depth1 <?php echo ($row['me_link']==$cur_url)?'on':''; ?>">
					<a href="<?php echo $row['me_link']; ?>" target="_<?php echo $row['me_target']; ?>" class="gnb_al_a"><?php echo $row['me_name'] ?></a>
					<i class="fa fa-chevron-down mobile_only" aria-hidden="true"></i>
					
					<?php
					$k = 0;
					foreach( (array) $row['sub'] as $row2 ){
						if($k == 0)
				        echo '<ul>'.PHP_EOL;
					?>
					
				    <li class="depth2"><a href="<?php echo $row2['me_link']; ?>" target="_<?php echo $row2['me_target']; ?>"><?php echo $row2['me_name'] ?></a></li>
				    
					<?php
					$k++;
					}   //end foreach $row2

					if($k > 0)
				    echo '</ul>'.PHP_EOL;
					?>
					
				</li>
				
				<?php
				$i++;
				}   //end foreach $row

				if ($i == 0) {  ?>
				
				<li class="gnb_empty">
				    Menu is being prepared.<?php if ($is_admin) { ?> <br>
				    <a href="<?php echo G5_ADMIN_URL; ?>/menu_list.php">관리자모드 &gt; 환경설정 &gt; 메뉴설정</a>
				    에서 설정하실 수 있습니다.<?php } ?>
				</li>
				<?php } ?>
				
			</ul>
			
			<!----------- 태블릿/모바일버전 SNS 버튼 영역 ----------->
			<div class="sns mobile_only">
				<ul>	
			        <br>
					<li><a id="channel-chat-button" href="http://pf.kakao.com/_MdxlzK/chat" onclick="void chatChannel();">카카오</a></li>
					
                    <!--
					<li><a href="javascript:">인스타</a></li>
					<li><a href="javascript:">유튜브</a></li>
                    -->
               
                    <br><br>
                    
                    <!-- 태블릿/모바일버전 국문페이지 링크 버튼 -->
			        <div> <a href="/">To Korean Web</a> </div>
				</ul>
			</div>
		</nav>
		
	</div><!----------- wrap 영역 종료 ----------->
	
	<hr class="hr01"><!----------- 메인메뉴,컨텐츠 구분선 ----------->
	
</header><!----------- 헤더 종료 } ----------->


<!-------------------------------------------------------------------------------------------------------------------------------------------------------------------------->	


<script type="text/javascript">
    
 // 웹 플랫폼 도메인 등 초기화한 앱의 설정이 그대로 적용됩니다.
 // 초기화한 앱에 현재 도메인이 등록되지 않은 경우 에러가 발생합니다.
// Kakao.init('c6dc22add6c016f2d3fa010aeb61d509') //'YOUR APP KEY===> 네이티브 앱 키: Android, iOS SDK에서 API를 호출할 때 사용===>적용함'
    
 function chatChannel() {
     
   Kakao.Channel.chat({
	 channelPublicId: '_MdxlzK',
   })
     
 }
    
</script>


<!----------- 검색창 영역 시작 { ----------->
<div class="hd_sch_wr">

	<div class="box">
	
		<header>
			<h2 style="display:inline;padding-top:5px;color:#fff;">Search in Boards</h2>
			<a href="javascript:" class="btn_close" title="Close"><img src="<?php echo G5_THEME_URL ?>/img/btn_close.png" alt=" " /></a>
		</header>
		
		<fieldset id="hd_sch">
			<legend>Search the entire site bulletin board</legend>
			<form name="fsearchbox" method="get" action="<?php echo G5_BBS_URL ?>/search.php" onsubmit="return fsearchbox_submit(this);">
                <input type="hidden" name="sfl" value="wr_subject||wr_content">
                <input type="hidden" name="sop" value="and">
                <label for="sch_stx" class="sound_only">Search term required</label>
                <input type="text" name="stx" id="sch_stx" maxlength="20" placeholder="Please enter a search term">
                <button type="submit" id="sch_submit" value="Search"><i class="fa fa-search" aria-hidden="true"></i><span class="sound_only">Search</span></button>
			</form>
        
	        <!----------- 검색창 Javascript 시작 ----------->
			<script>
                
                function fsearchbox_submit(f) {
                    if (f.stx.value.length < 2) {
                        alert("Please enter at least two characters for your search term.");
                        f.stx.select();
                        f.stx.focus();
                        return false;
                    }

                    // 검색에 많은 부하가 걸리는 경우 이 주석을 제거하세요.
                    var cnt = 0;
                    for (var i=0; i<f.stx.value.length; i++) {
                        if (f.stx.value.charAt(i) == ' ')
                        cnt++;
                    }

                    if (cnt > 1) {
                        alert("You can enter only one space in the search term for quick search..");
                        f.stx.select();
                        f.stx.focus();
                        return false;
                    }

                    return true;
                }
                
                $(function(){
                    
                    // 검색 아이콘 클릭시 검색창 열림
                    $("#header .btn_search").on("click", function(){
                        $(".hd_sch_wr").addClass('on');
                    });
                    
                    // 검색창의 닫기버튼 클릭시 검색창 닫힘
                    $(".hd_sch_wr .btn_close").on("click", function(){
                        $(".hd_sch_wr").removeClass('on');
                    });

                });
                
			</script>
			<!----------- 검색창 Javascript 종료 ----------->
	
		</fieldset>
		
	</div>
	
	<div class="bo_sch_bg"></div>
	
</div><!----------- 검색창 영역 종료 } ----------->
    
    
<!-------------------------------------------------------------------------------------------------------------------------------------------------------------------------->
     
  
<script>
    
var isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent) ? true : false;
    
/* ========================= 메인메뉴 Javascript 시작 ========================= */
    
$(function(){
    
    /* ============= PC버전 메인메뉴 ============= */
	if(!isMobile){
        
        // 1depth에 마우스오버시 GNB가 열리면서 2depth/회원메뉴 리스트(로그인/로그아웃/어드민 등등)가 보여짐
		$("#header .depth1").on("mouseover", function(){
			$("#header").addClass('down');
			$("#header .user").addClass('on');
		});
        
        // 헤더 영역에서 마우스아웃시 GNB/회원메뉴 리스트(로그인/로그아웃/어드민 등등)가 닫힘
		$("#header").on("mouseleave", function(){
			$("#header").removeClass('down');
			$("#header .user").removeClass('on');
		});
        
        // 회원메뉴 아이콘에 마우스오버시 GNB가 열리면서 2depth/회원메뉴 리스트(로그인/로그아웃/어드민 등등)가 보여짐
		$("#header .btn_user").on("mouseover", function(){
			$("#header").addClass('down');
			$("#header .user").addClass('on');		
		});
        
	};
    
    /* ============= 태블릿/모바일버전 메인메뉴 ============= */
	if(isMobile){
        
        // 회원메뉴 아이콘 클릭시 회원메뉴 리스트(로그인/로그아웃/어드민 등등)가 토글됨
        $("#header .btn_user").on("click", function(){
			$("#header .user").toggleClass('on');
		});
        
        // GNB버튼 클릭시 GNB가 열리면서 GNB버튼 토글됨(열림/닫힘버튼)
        $("#header .btn_gnb").on("click", function(){
            $("#header .user").removeClass('on');
            $(this).toggleClass('on');
            $("#header .gnb").toggleClass('on');
            $("#header .dimmed").toggleClass('on');
        });

        // GNB의 1depth 클릭시 2depth가 보여지고, 다른 1depth를 클릭하면 기존의 1depth의 2depth가 닫힘
        $("#header .gnb").find('> ul > li > i').on("click", function(){
            if($(this).parent().hasClass('on')){
                $(this).parent().removeClass('on');
            } else{
                $("#header .gnb .depth1").removeClass('on');
                $(this).parent().toggleClass('on');
            }
        });
        
    };
    
});
    
/* ========================= 메인메뉴 Javascript 종료 ========================= */
    
$(function(){	
	$(window).scroll(function(){
		var top = $(window).scrollTop();
		var subTop = $('#sub_visual').height() - $('#header').height(); // 200327 서브비주얼 영역
		if( top > 0 ){
			$('#header').addClass('on');
			$('#rightQuick').addClass('on');
		}else{
			$('#header').removeClass('on');
			$('#rightQuick').removeClass('on');
		};
		if( top > subTop ){ // 200327 서브비주얼 영역 넘었을 시 fix클래스 추가
			$('#left_area').addClass('fix');
		}else{
			$('#left_area').removeClass('fix');
		};
	});
	$(window).scroll();		
});
    
</script>


<!-------------------------------------------------------------------------------------------------------------------------------------------------------------------------->


<?php if (!defined("_INDEX_")) { ?>

<?php
	################################################################################################
	##### 관리자 메뉴설정을 아래와 같이 한다고 가정하에 왼쪽 메뉴 설정  (bo_table 기준으로 작업)
	################################################################################################
	##### 게시판이 아닐때 : /theme/a01/sub01/sub01.php
	##### 게시판 일때 : /bbs/board.php?bo_table=product01
	################################################################################################

	// 게시판이 아니고 co_id 값이 없을 때
	if($bo_table == "" && $co_id == "") {
		$url = preg_replace('/(qawrite\.php|qaview\.php)/i', 'qalist.php', $_SERVER['SCRIPT_NAME']); // 일대일상대 적용
	// 게시판일때
	}else{
		$base_url = preg_replace('/write\.php/i', 'board.php', $_SERVER['SCRIPT_NAME']);
		// 인터넷 주소창에 "&" 없는 경우 - 글목록/글등록/글수정
		if(strpos($_SERVER['QUERY_STRING'], "&") === false) {
    		$url = $base_url."?".$_SERVER['QUERY_STRING'];
		// 인터넷 주소창에 "&" 있는 경우 - 글보기, 글검색
		}else{
			$query_string = preg_replace('/(w=u&)/i', '', $_SERVER['QUERY_STRING']);
			$url = $base_url."?".substr($query_string, 0, strpos($query_string, "&"));
		}
	}

    $sql = " select * from {$g5['menu_table']} where length(me_code) = '4' and me_link like '%".$url."%' and me_use = '1' order by me_order, me_id ";
	$query = sql_query($sql, false) ;
	$total  = sql_num_rows($query) ;
	$list = array() ;
	if($total > 0) {
		$row = sql_fetch_array($query);
		$keyword =  $row['me_code'] ;
		if(strlen($keyword) >= 2) {
			$filter_keyword = substr($keyword, 0, 2);
			$sql = " select * from {$g5['menu_table']} where me_code like '{$filter_keyword}%' and me_use = '1' order by me_order, me_code ";
			$query = sql_query($sql, false);
			$num = sql_num_rows($query);
			for($i=0; $i < $num;$i++) {
				$list[$i] = sql_fetch_array($query);
			}
		}
	}	
	$pageNum = substr($row['me_code'], 0, 2);
    if (count($list) > 0) {
?>
<div id="sub_visual" class="sub<?=$pageNum?>">
	<div class="bg"></div>
	<?php for($i=0;$i<count($list);$i++) { if ($i == 0) { ?>	 
    <h2><?php echo $list[$i]['me_name']?></h2>
    <?php } } ?>
</div>

<!-- 200506 Mark / islro navigation추가 -->
<div id="sub_navi">
	<div class="wrap">
		<ul>
			<li><a href="/"><i class="fa fa-home" aria-hidden="true"></i> HOME</a></li>
			<?php for($i=0;$i<count($list);$i++) { if ($i == 0) { ?>
			<li><a href="<?php echo $list[$i]['me_link']?>">&gt; <?php echo $list[$i]['me_name']?></a></li>
			<li>&gt; <?php echo $g5['title']?></li>
			<?php } } ?>
		</ul>
	</div>
</div>
<?php } else  { 
$str = explode("/",$_SERVER["REQUEST_URI"]); 
	switch($str[3]){ // url 3번째 폴더명 기준으로 navigation경로 수동 지정
		case "manage" : $pageName="제공서비스"; $depth1="> 제공서비스"; $depth1="> 제공서비스"; $depth2="> 시스템인증"; $depth2_link="/theme/rt_igcert/manage/igc_006.php"; break; 
		case "medical" : $pageName="제공서비스"; $depth1="> 제공서비스"; $depth2="> 시험"; $depth2_link="/theme/rt_igcert/medical/igc_026.php"; break; 
		case "edu1" : $pageName="제공서비스"; $depth1="> 제공서비스"; $depth2="> 교육"; $depth2_link="/theme/rt_igcert/edu1/igc_033.php"; break; 
		case "product" : $pageName="제공서비스"; $depth1="> 제공서비스"; $depth2="> 제품인증"; $depth2_link="/theme/rt_igcert/product/igc_069.php"; break; 
		case "aduits" : $pageName="제공서비스"; $depth1="> 제공서비스"; $depth2="> 심사원인증"; $depth2_link="/theme/rt_igcert/aduits/igc_104.php"; break; 
		case "global" : $pageName="제공서비스"; $depth1="> 제공서비스"; $depth2="> 연수기관"; $depth2_link="/theme/rt_igcert/global/igc_105.php"; break; 
	} 
?> 
<div id="sub_visual" class="sub20">
    <div class="bg"></div>
	<h2>
		<? 
			if($pageName != "") {
				echo $pageName;
			}else{
				echo $g5['title'];
			}
		?>
	</h2>
</div>
<div id="sub_navi">
	<div class="wrap">
		<ul>
			<li><a href="/"><i class="fa fa-home" aria-hidden="true"></i> HOME</a></li>
			<li><a href="/theme/rt_igcert/manage/igc_006.php"><?php echo $depth1?></a></li>
			<li><a href="<?php echo $depth2_link?>"><?php echo $depth2?></a></li>
			<li>&gt; <?php echo $g5['title']?></li>
		</ul>
	</div>
</div>
<?php } ?>
<!-- /200506 islro navigation추가 -->

<!---기존 코드, 서브메뉴===>가로형태 시작 -?php include(G5_THEME_PATH.'/snb.php'); ?  기존 코드, 서브메뉴===>가로형태 종료----->
<div id="contents">	
	<div class="wrap">
	<!-----컨텐츠 영역 시작 2020년3월23일, 서브페이지 좌측 영역과 우측 컨텐츠 영역 구분하기 위해 코딩 수정------>
	<!-------- 현재 서브페이지 서브이미지 바로 아래 있던 코드 시작 ------>
		<div id="contents_area">
		<!-----h2 class="container_title"><//?php echo get_head_title($g5['title']); ?></h2---->
		<!------기존에 있던 코드 종료------->	
			<div id="left_area">
				<?php include(G5_THEME_PATH.'/snb.php'); ?>
			</div>
			<div id="right_area">
				<h2 class="container_title"><?php echo get_head_title($g5['title']); ?></h2>	
<? } ?>
